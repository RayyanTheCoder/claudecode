...
const CACHE = "beam-v1";
const ASSETS = ["./", "./index.html", "./beam.html", "./manifest.webmanifest",
  ...
  if (isDocument(e.request, url)) {
    e.respondWith(
      fetch(e.request).then(res => {
        ...
        return res;
      }).catch(() => caches.match(e.request).then(hit => hit || caches.match("./beam.html") || caches.match("./")))
    );
    return;
  }
